...
public class ContainerFluidDiskWorkbench extends Container
{
    ...
    public static final ContainerType<ContainerFluidDiskWorkbench> CURRENT_CONTAINER = null;
	
	public TileEntityFluidDiskWorkbench tile;
	PlayerInventory playerInventory;
	
	public ContainerFluidDiskWorkbench(int windowId, PlayerInventory _playerInventory)
	{
		this(windowId, new TileEntityFluidDiskWorkbench(), _playerInventory);
	}
	
	public ContainerFluidDiskWorkbench(int windowId, TileEntityFluidDiskWorkbench _tile, PlayerInventory _playerInventory)
	{
	...
	public ItemStack transferStackInSlot(PlayerEntity playerIn, int index)
	{
		 ...
		 else if(DiskUtils.validFluidPart(from))
		 {
			 Slot go = getSlot(36);
			 if(go.getStack().isEmpty() || DiskUtils.validFluidPart(go.getStack()))
			 {
				 int total = go.getStack().getCount() + from.getCount();
				 if(total <= go.getStack().getMaxStackSize())
				 {
