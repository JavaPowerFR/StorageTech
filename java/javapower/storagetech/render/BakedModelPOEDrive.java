...
public class BakedModelPOEDrive extends DelegateBakedModel
{
    ...
    private final LoadingCache<CacheKey, List<BakedQuad>> cache = CacheBuilder.newBuilder().build(new CacheLoader<CacheKey, List<BakedQuad>>()
    {
        ...
        {
            Direction facing = key.state.get(STBlocks.blockPOEDrive.getDirection().getProperty());
    ...
    public BakedModelPOEDrive(IBakedModel base, IBakedModel disk, IBakedModel diskNearCapacity, IBakedModel diskFull, IBakedModel diskDisconnected)
    {
        super(base);
