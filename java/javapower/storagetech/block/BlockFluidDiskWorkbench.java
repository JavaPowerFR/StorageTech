...
public class BlockFluidDiskWorkbench extends BBaseContainer
{
	public BlockFluidDiskWorkbench()
	{
		super(STBlocks.DEFAULT_BLOCK_PROPERTIES, "fluiddiskworkbench");
	}
			...
			thisItem = new BlockItem(this, new Item.Properties().group(StorageTech.creativeTab))
			{
				...
				public void addInformation(ItemStack stack, World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn)
				{
					if(stack.hasTag())
					{
						...
						tooltip.add(new TranslationTextComponent(I18n.format("storagetech.tooltip.memory")+": "+Tools.longFormatToString(nbt.getLong("memory"))+" VFT"));
						if(nbt.contains("prosses"))
							tooltip.add(new TranslationTextComponent(I18n.format("storagetech.tooltip.inprocess")+": "+nbt.getBoolean("prosses")).func_230530_a_(Styles.GRAY));
					}
				...
				protected boolean placeBlock(BlockItemUseContext context, BlockState newState)
				{
			        ...
			        if (newState.getBlock() == this.getBlock())
			        {
			            ...
			            if(stack.hasTag())
			            {
			            	...
			            	if(tile != null && tile instanceof TileEntityFluidDiskWorkbench)
			            	{
			            		((TileEntityFluidDiskWorkbench)tile).readFromNBT(stack.getTag());
			            	}
			            }
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult p_225533_6_)
	{
		...
		if(!worldIn.isRemote)
		{
			...
			(
					...
					new PositionalTileContainerProvider<TileEntityFluidDiskWorkbench>
						(
								((TileEntityFluidDiskWorkbench) worldIn.getTileEntity(pos)).getDisplayName(),
								(tile, windowId, inventory, p) -> new ContainerFluidDiskWorkbench(windowId, tile, inventory),
								pos
						),
						pos
			);
