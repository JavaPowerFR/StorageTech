...
public class BlockGasImporter extends CableBlock
{
	protected Item thisItem;
	public static String raw_name = "gasimporter";
    ...
    {
        return new TileEntityGasImporter();
    }
    ...
    public ActionResultType onBlockActivated(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockRayTraceResult hit)
    {
        if (!world.isRemote && CollisionUtils.isInBounds(getLineShape(state), pos, hit.getHitVec()))
        {
            return NetworkUtils.attemptModify(world, pos, hit.getFace(), player, () -> NetworkHooks.openGui(
                ...
                new PositionalTileContainerProvider<TileEntityGasImporter>(
                    new TranslationTextComponent("gui.storagetech.gas_importer"),
                    (tile, windowId, inventory, p) -> new ContainerGasImporter(tile, player, windowId),
                    pos
                ),
                pos
            ));
