...
public class BakedModelGasDrive extends DelegateBakedModel
{
    ...
    private final LoadingCache<CacheKey, List<BakedQuad>> cache = CacheBuilder.newBuilder().build(new CacheLoader<CacheKey, List<BakedQuad>>()
    {
        ...
        {
            Direction facing = key.state.get(MKBlocks.blockGasDrive.getDirection().getProperty());
    ...
    public BakedModelGasDrive(IBakedModel base, IBakedModel disk, IBakedModel diskNearCapacity, IBakedModel diskFull, IBakedModel diskDisconnected)
    {
        super(base);
    ...
    {
        DiskState[] diskState = data.getData(TileEntityGasDrive.DISK_STATE_PROPERTY);
